...
class AIEnhancedLiveMarketTest:
    ...
    def evaluate_pattern_quality(self, result, current_hour: float) -> Tuple[float, str]:
        ...
        # Kernel alignment
        if result.filter_states.get('kernel_bullish', False) and result.signal > 0:
            score += 1
            factors.append("Kernel confirms direction")
        elif result.filter_states.get('kernel_bearish', False) and result.signal < 0:
            score += 1
            factors.append("Kernel confirms direction")
    ...
    def _enter_long_position(self, symbol: str, bar_data: Dict, result, score: float, explanation: str):
        ...
        signal_info = {
            ...
            'target_2': entry_price + 40,  # ₹40 second target
        ...
        print(f"   Stop Loss: ₹{stop_loss:.2f} (-1.0%)")
        print(f"   Target 1: ₹{signal_info['target_1']:.2f} (+₹20) [50% exit]")
        print(f"   Target 2: ₹{signal_info['target_2']:.2f} (+₹40) [full exit]")
        ...
        print(f"   Max Profit: ₹{40 * position_size:.2f}")
        print(f"   Risk/Reward: 1:{40/(entry_price - stop_loss):.1f}")
    ...
    def _enter_short_position(self, symbol: str, bar_data: Dict, result, score: float, explanation: str):
        ...
        print(f"   Max Profit: ₹{40 * position_size:.2f}")
        print(f"   Risk/Reward: 1:{40/(stop_loss - entry_price):.1f}")
    ...
    def _print_final_summary(self):
        ...
        if total_trades > 0:
            ...
            trades_df['entry_hour'] = trades_df['entry_time'].dt.hour + trades_df['entry_time'].dt.minute / 60
